...
reference_file <- paste0(user_directory,
                         "/Code Reference/",
                         "Analysis Reference 2024-08-12.xlsx")
...
# if (cp_eval){
#   kable_cp_tat(x = bloodbank_tat_output[[2]])
# } else {
#   asis_output(
#     paste("<i><b><span style='color:red'>",
#           "Missing Sunquest and/or SCC data. Please check that file is saved properly.",
...
if(form_eval){
  ...
  good_catch <- NULL
  never_events_comments <- NULL
  comments_clinical_labs <- NULL
}
...
if(form_eval & !is.null(formatted_clinical_labs3)){
...
if(form_eval & !is.null(comments_clinical_labs)){
...
if(form_eval & !is.null(formatted_lis_new2)){
  formatted_lis_new2 %>%
      select(everything()) %>%
      kable(escape = F, align = "c",
...
if(form_eval & !is.null(never_events_only)){
  never_events_only %>%
    select(everything()) %>%
    kable(escape = F, align = "c",
          col.names = c(
...
if(form_eval & !is.null(never_events_comments)){
  never_events_comments %>%
    select(everything()) %>%
    kable(escape = F, align = "c",
...
if(form_eval & !is.null(good_catch)){
  good_catch %>%
    select(everything()) %>%
    kable(escape = F, align = "c",
