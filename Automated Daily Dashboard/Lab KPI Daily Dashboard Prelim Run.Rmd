...
#             output_file = paste0("/Pathology/Prelim Dashboard Output/",
...
cp_test_divisions <- 
  unique(unique(cp_scc_test_codes[, c("TEST", "DIVISION")]))
...
SITES <- c("MSB","PACC","MSW","MSH","MSM","MSSN","MSQ","NYEE")
...
epic_data_raw <- tbl(conn,"PATH_ORDER_RESULT_V") %>%
  filter(RESULT_DATE_TIME >=  TO_DATE(yesterday,'YYYY-MM-DD hh24:mi:ss')) %>%
  collect()
if(nrow(epic_data_raw)== 0){
    epic_data_raw <- NULL
}
...
cp_processed <- rbind(scc_processed)
...
# Blood Bank
bloodbank_tat_output <- summarize_cp_tat(x = cp_summary,
                                        lab_division = "Blood Bank")

blood_bank_summary <- bloodbank_tat_output[[1]]
blood_bank_cast <- bloodbank_tat_output[[2]]

```

```{r Import AP custom functions, echo=FALSE, message=FALSE, warning=FALSE}
#######
...
cyto_prep <- function(epic_data, pp_data,resulted_date) {
  if (is.null(epic_data) || is.null(pp_data) || 
    ...
    cyto_data <- merge(x = cyto_data, y = tat_targets_ap,
                          all.x = TRUE,by.x = c("spec_group", "PATIENT_SETTING") ,by.y = c("SPEC_GROUP", "PATIENT_SETTING"))
    ...
    summarized_table <- cyto_data %>%
      ...
      summarise(no_cases_signed = n(),
                ...
                lab_metric_within_target = as.numeric(format(
                  round(
                    sum(Received_to_signed_out <= RECEIVED_TO_SIGNED_OUT_DAYS,
                        na.rm = TRUE) / sum(
...
patho_prep <- function(raw_data,resulted_date) {
  if (is.null(raw_data) || nrow(raw_data) == 0) {
    ...
    pp_data_gi_codes <- merge(x = raw_data, y = gi_codes, all.x = TRUE,by.x ="Spec_code",by.y = "SPEC_CODE")
    ...
    patient_setting_with_targets <- merge(x = sp_data_patient_setting, y = tat_targets_ap,
                                          all.x = TRUE, by.x = c("spec_group", "PATIENT_SETTING"),
                                          by.y = c("SPEC_GROUP", "PATIENT_SETTING"))
  ...
  if (is.null(cyto_backlog_raw) || nrow(cyto_backlog_raw) == 0) {
    ...
    summarized_table <- cyto_backlog %>%
      ...
      summarise(cyto_backlog = format(
        round(
          sum(
            backlog > RECEIVED_TO_SIGNED_OUT_DAYS,
            na.rm = TRUE), 0)),
        ...
        percentile_25th =
          format(
            ceiling(
              quantile(
                backlog[backlog > RECEIVED_TO_SIGNED_OUT_DAYS],
...
get_efficiency_indicators_cytology <- function(summarized_data){
  ...
  efficiency_indicator_cytology_tab <- left_join(left_join(efficiency_indicator_cytology_tab,
                                                           vol_cases_signed),
                                                 received_to_signed) %>%
    mutate(Target = paste0("<= ",as.character(RECEIVED_TO_SIGNED_OUT_DAYS)," Days")) %>%
  ...
  if(is.null(surgical_pathology_ei_data)){
    ...
    kable(surgical_pathology_ei_data, format = "html", escape = FALSE, align = "c",
      ...
      add_header_above(c(" " = 1,
                       ...
                       background = c("white", "#00AEEF", "#221f72"),
                       color = "white", line = FALSE, font_size = 13) %>%
      column_spec(column = 2:10, background = "#E6F8FF",color = "black") %>%
      column_spec(column = 11:num_col, background = "#EBEBF9",color = "black") %>%
      row_spec(row = 0, font_size = 13) %>%
      collapse_rows(columns = c(1, 2))
...
### Blood Bank
#### *Blood Bank KPI (Specimens Signed Out on `r result_date_text`)*
<h5>Status Definitions: <span style = "color:red">Red:</span> <80%, 
<span style = "color:orange">Yellow:</span> >=80% & <90%,
<span style = "color:green">Green:</span> >=90%</h5>
```{r Blood Bank Efficiency Indicators, echo = FALSE, warning = FALSE, message = FALSE}
if (cp_eval) {
  if (is.null(blood_bank_summary)) {
    bloodbank_tat_vol_cast <- NULL
    } else {
      # Ensure all Microbiology RRL test, site, and priority combinations are included
      bloodbank_vol_templ <- tat_dashboard_templ %>%
        filter(Division == "Blood Bank") %>%
        mutate(Incl = NULL,
               Division = NULL)
      
      blood_bank_vol_df <- blood_bank_summary %>%
        mutate(Metric = "ResultedVolume") %>%
        rename(Value = ResultedVolume) %>%
        select(-Division, -ResultedVol_ReceiveTAT, -ResultedVol_CollectTAT,
               -ReceiveResultInTarget, -CollectResultInTarget,
      ...
      original_length <- ncol(blood_bank_vol_df)
      
      missing_cols <- colnames(blood_bank_cast)[
        seq(original_length + 1, ncol(blood_bank_cast))]
      
      blood_bank_vol_df[missing_cols] <- ""
      
      colnames(blood_bank_vol_df) <- colnames(blood_bank_cast)
      
      bloodbank_tat_vol_cast <- rbind(blood_bank_cast, blood_bank_vol_df)
      
      bloodbank_tat_vol_cast <- bloodbank_tat_vol_cast %>%
        arrange(Test, DashboardPriority, ReceiveResultTarget)
      }
  
  kable_cp_tat(x = bloodbank_tat_vol_cast)
} else {
  asis_output(
    paste("<i><b><span style='color:red'>",
          "Missing Sunquest and/or SCC data. Please check that file is saved properly.",
...
```{r Cytology Signed Out Cases, echo=FALSE, warning=FALSE, message=FALSE, eval = TRUE}
...
#              stringsAsFactors = FALSE)
definition_table <- tbl(conn,"LAB_KPI_DEFINITIONS") %>%
  collect()
...
test_codes_kable <- cp_scc_test_codes
test_codes_kable <- test_codes_kable %>%
  select(DIVISION, TEST, SCC_TEST_ID) %>%
...
test_codes_kable %>%
  ...
  kable_styling(bootstrap_options = "hover", position = "center",
                font_size = 11, full_width = FALSE) %>%
  column_spec(column = c(1, 2, 3), border_right = "thin solid lightgray") %>%
  row_spec(row = 0, font_size = 13, background = "#221f72", color = "white") %>%
  collapse_rows()
