...
cp_user_path <- paste0(user_directory, "/CP Repositories/DailyRepo/*.*")
  ...
  readRDS(
    file =
      choose.files(default = cp_user_path,
                   caption = "Select Clinical Pathology Historical Repository"))
...
cp_hist$ResultDate <- as.Date(cp_hist$ResultDate, format = "%m/%d/%y")
lookback_period <- 30
start_date <- max(cp_hist$ResultDate) - (lookback_period - 1)
...
cp_dashboard_tat_summary <- cp_hist %>%
  ...
  mutate(ReceiveResultDefects = ResultedVol_ReceiveTAT - ReceiveResultInTarget,
         CollectResultDefects = ResultedVol_CollectTAT - CollectResultInTarget,
         ReceiveResultPercent = ReceiveResultInTarget / ResultedVol_ReceiveTAT,
         CollectResultPercent = CollectResultInTarget / ResultedVol_CollectTAT,
         SafeThreshold = ifelse(Division %in% c("Microbiology RRL"), 1.0,
                                 ifelse(Division %in% c("Infusion"), 0.9,
                                        0.95)),
         NotSafeThreshold = ifelse(Division %in% c("Microbiology RRL"), 0.9,
                                    0.8))

cp_dashboard_vol_summary <- cp_hist %>%
...
cp_dashboard_tat_filter <- cp_dashboard_tat_summary %>%
  filter(!(DashboardPriority == "Other" |
             DashboardSetting == "Other" | 
             ((Test == "Troponin" | Test == "Lactate WB") &
                DashboardSetting == "Amb") |
             (Test == "C. diff" & DashboardSetting == "Amb")))

# Convert characters to factors and arrange
cp_dashboard_tat_filter <- cp_dashboard_tat_filter %>%
  mutate(Site = factor(Site,
         ...
         Test = factor(Test,
                       ...
                       ordered = TRUE),
         DashboardPriority = factor(DashboardPriority,
                                    levels = dashboard_priority_order,
                                    ordered = TRUE),
         DashboardSetting = factor(DashboardSetting,
...
cp_tat_percentage_graph <- 
  function(data, test_name, site_name, lab_priority, metric) {
    # Determine metric for visualization
    metric_name <- ifelse(metric == "ReceiveResultPercent",
                          "Receive to Result TAT",
                          ifelse(metric == "CollectResultPercent",
                                 "Collect to Result TAT", "Error"))
  # 
  ggplot() +
    # Create bounds for "not safe" performance
    geom_rect(aes(xmin = start_date - 1,
                  xmax = max(data$ResultDate) + 1,
                  ymin = -Inf,
                  ymax = min(data$NotSafeThreshold),
                  fill = "Not Safe",
                  alpha = "Not Safe")) +
    # Create bounds for "at risk" performance
    geom_rect(aes(xmin = start_date - 1,
                  xmax = max(data$ResultDate) + 1,
                  ymin = min(data$NotSafeThreshold),
                  ymax = min(data$SafeThreshold),
                  fill = "At Risk",
                  alpha = "At Risk")) +
    # Create bounds for "safe" performance
    geom_rect(aes(xmin = start_date - 1,
                  xmax = max(data$ResultDate) + 1,
                  ymin = min(data$SafeThreshold),
                  ymax = Inf,
                  fill = "Safe",
                  alpha = "Safe")) +
    # Plot data for the  last 30 days
    geom_point(data = data[data$ResultDate >= start_date &
                             data$Test == test_name &
                             data$Site == site_name &
                             data$DashboardPriority == lab_priority, ],
               aes_string(x = "ResultDate",
                          y = metric,
                          color = "DashboardSetting",
                          shape = "DashboardSetting"),
               size = 3) +
    geom_line(data = data[data$ResultDate >= start_date &
                            data$Test == test_name &
                            data$Site == site_name &
                            data$DashboardPriority == lab_priority, ],
              aes_string(x = "ResultDate",
    ...
    scale_x_date(limits = c(start_date - 1, max(data$ResultDate) + 1),
                 breaks = seq(start_date, max(data$ResultDate), by = 3),
...
cp_resulted_volume_graph <- 
  function(data, test_name, site_name) {
    ggplot() +
      geom_point(data = data[data$ResultDate >= start_date &
                               data$Test == test_name &
                               data$Site == site_name, ],
                 aes(x = ResultDate,
                     y = ResultedVolume,
                     color = DashboardPriority,
                     shape = DashboardPriority),
                 size = 3) +
      geom_line(data = data[data$ResultDate >= start_date &
                              data$Test == test_name &
                              data$Site == site_name, ],
...
cp_tat_and_volume_plot <- 
  function(data_tat, test_name, site_name, lab_priority, metric) {
    # 
    ...
    ggplot() +
      ...
      geom_rect(aes(xmin = start_date - 1,
                    xmax = max(data_tat_plot$ResultDate) + 1,
                    ymin = -Inf,
                    ymax = min(data_tat_plot$NotSafeThreshold),
                    fill = "Not Safe",
                    alpha = "Not Safe")) +
      geom_rect(aes(xmin = start_date - 1,
                    xmax = max(data_tat_plot$ResultDate) + 1,
                    ymin = min(data_tat_plot$NotSafeThreshold),
                    ymax = min(data_tat_plot$SafeThreshold),
                    fill = "At Risk",
                    alpha = "At Risk"),
                show.legend = FALSE) +
      geom_rect(aes(xmin = start_date - 1,
                    xmax = max(data_tat_plot$ResultDate) + 1,
                    ymin = min(data_tat_plot$SafeThreshold),
                    ymax = 1,
                    fill = "Safe",
                    alpha = "Safe"),
                show.legend = FALSE) +
      ...
      geom_point(data = data_tat_plot,
                 aes_string(x = "ResultDate",
                            y = metric,
                            color = "DashboardSetting",
                            shape = "DashboardSetting"),
                 size = 3) +
      geom_line(data = data_tat_plot,
                aes_string(x = "ResultDate",
                           y = metric,
                           color = "DashboardSetting"),
                linetype = "dashed") +
      # Plot resulted volume on secondary axis
      geom_point(data = data_vol_plot,
                 aes(x = ResultDate,
                     y = ResultedVolume / scale,
                     size = "Volume"),
                 color = "#959595",
                 shape = 18) +
      geom_line(data = data_vol_plot,
                aes(x = ResultDate,
                    y = ResultedVolume / scale,
                    linetype = "Volume"),
                color = "#959595") +
      ...
      guides(color =
               guide_legend(
                 order = 1,
                 nrow = length(unique(data_tat_plot$DashboardSetting)),
                 title.vjust = 1),
             shape =
               guide_legend(
                 order = 1,
      ...
      scale_x_date(limits = c(start_date -1, max(data_tat_plot$ResultDate)+1),
                   breaks = seq(start_date, max(data_tat_plot$ResultDate),
                                by = 3),
                   date_minor_breaks = "1 day",
...
cp_stratified_data_for_plots <- function(tat_data, test) {
  ...
  for (site in sites) {
    ...
    for (priority in priorities) {
      ...
      print(cp_tat_and_volume_plot(data_tat = priority_df,
                                   test_name = test,
                                   site_name = site,
    ...
    print(cp_resulted_volume_graph(data = cp_dashboard_vol_summary,
                                   test_name = test,
                                   site_name = site))
  }
}
...
cp_dashboard_defect_summary <- cp_dashboard_tat_filter %>%
  ...
  summarize(ResultedVolume = sum(ResultedVolume),
            ResultedVol_ReceiveTAT = sum(ResultedVol_ReceiveTAT),
            ReceiveResultInTarget = sum(ReceiveResultInTarget),
            CollectResultInTarget = sum(CollectResultInTarget),
            ReceiveResultDefects =
              ResultedVol_ReceiveTAT - ReceiveResultInTarget,
            ReceiveResultDefectRate =
              ReceiveResultDefects / ResultedVol_ReceiveTAT)

cp_dashboard_defect_summary <- cp_dashboard_defect_summary %>%
  filter(!is.na(ReceiveResultDefectRate))
...
cp_receive_result_pchart <-
  function(data_defect, test_name, site_name, lab_priority) {
    #
    # Subset turnaround time and volume data based on test, site, and priority
    ...
    center_line <- 
      mean(QC_Lines(data = data_defect_plot$ReceiveResultDefectRate,
                    n = data_defect_plot$ResultedVol_ReceiveTAT,
                    method = "p")$pBar)
    #
    # Create plot and standard settings
  ggplot(data = data_defect_plot,
         aes(x = ResultDate,
             y = ReceiveResultDefectRate,
             n = ResultedVol_ReceiveTAT)) +
    ...
    geom_label(x = max(data_defect_plot$ResultDate)+1,
               y = center_line,
               vjust = 1.2,
               hjust = 1,
    ...
    theme(plot.title = element_text(hjust = 0.5),
          ...
          axis.text.x = element_text(angle = 30, hjust = 1)) +
    # Specify axis properties
    scale_x_date(limits = c(start_date - 1,
                            max(data_defect_plot$ResultDate) + 1),
                 breaks = seq(start_date, max(data_defect_plot$ResultDate),
                              by = 3),
                 date_minor_breaks = "1 day",
                 date_labels = "%m/%d/%y",
                 expand = c(0, 0, 0, 0)) +
...
cp_stratified_data_for_pcharts <- function(defect_data, test) {
  ...
  for (site in sites) {
    ...
    for (priority in priorities) {
      ...
      avg_resulted_volume <- 
        mean(priority_df$ResultedVol_ReceiveTAT[
          priority_df$ResultDate >= start_date])
      # print(avg_resulted_volume)
      if (avg_resulted_volume >= 50) {
        print(cp_receive_result_pchart(data_defect = priority_df,
                                       test_name = test,
                                       site_name = site,
                                       lab_priority = priority))
      } else {
        cat(paste('\n<center><h3><i>',
                  ...
                  test,
                  priority,
                  "Labs: Insufficient volume for this analysis (n<50).</h3> \n <h4>(Average resulted volume over last 30 days:",
                  round(avg_resulted_volume, digits = 0),
                  "specimens.)",
                  '</i></h4></center>\n\n<br><br>'))
      }
    }
...
#Exclude weekends - based on the discussion with the leadership since there will
#be mostly no specimens on weekdays
ap_hist_repo_wkdy <-
  ap_hist_repo[which(
...
#Makes sure all the dates in the data are in date format for the calculations
ap_hist_repo_wkdy[c("Signed_out_date_only",
                    "acc_date_only",
                    "report_date_only")] <-
  lapply(ap_hist_repo_wkdy[c("Signed_out_date_only",
                             "acc_date_only",
                             "report_date_only")],
         as.Date)

#Makes sure all the variables in the data are in numeric for the calculations
ap_hist_repo_wkdy[c("Lab_metric_target",
                    ...
                    "Patient_metric_med",
                    "cyto_acc_vol")] <-
  lapply(ap_hist_repo_wkdy[c("Lab_metric_target",
                             "Patient_metric_target",
                             "No_cases_signed_out",
                             "Lab_metric_avg",
                             "Lab_metric_med",
...
#Add an extra column to change the facilities for Cyto GYN and Cyto Non-GYN
...
#pick the last 30 week days
ap_reporting_30_day_period <- ap_ordered_dates[1:30]
#get the first day in the reporting period
ap_first_day <- ap_ordered_dates[30]
#get the last day in the reporting period
ap_last_day <- ap_ordered_dates[1]
...
#summarized the data to plot it correctly for TAT within target.
#This table summarizes the historical data for the receive to result (rtr). It
#makes sure that the facility for cyto gyn and cyto nongyn is centralized while
#for pathology it is based on the home hospital
ap_hist_30_day_rtr <-
  summarise(group_by(ap_hist_30_day,
            ...
            Lab_metric_within_target =
              format(round(sum(
                (Lab_metric_within_target * No_cases_signed_out) /
                  sum(No_cases_signed_out), na.rm = TRUE), 2)),

            Patient_metric_avg =
              format(round(sum((Patient_metric_avg * No_cases_signed_out) /
...
#summarized the data to plot it correctly for average collect to result
#This table summarizes the historical data for the collect to result (ctr). It
#makes sure that the facility is based on the home hospital
ap_hist_30_day_ctr <-
  summarise(group_by(ap_hist_30_day,
                     Signed_out_date_only,
                     Spec_group,
...
#summarized the data to plot it correctly for rtr volume plots.
#This table summarizes the historical data for the receive to result (rtr). It
#makes sure that the facility for cyto gyn and cyto nongyn is centralized while
#for pathology it is based on the home hospital
ap_hist_30_day_vol <-
...
#summarized the data to plot it correctly for ctr volume plots
#This table summarizes the historical data for the collect to result (ctr). It
#makes sure that the facility is based on the home hospital
  ...
  function(ap_hist_30_day_rtr,
    ...
    metric_name <-
      ifelse(metric == "Lab_metric_within_target", "Receive to Result",
             ifelse(metric == "Patient_metric_avg", "Collect to Result",
                    "Error"))
    ...
    test_name <-
      ...
      ggplot() +
      # Plot safe, at risk, not safe thresholds
      geom_rect(aes(xmin = ap_first_day - 1,
                    xmax = ap_last_day + 1,
                    ymin = -Inf,
                    ymax = 0.8,
                    fill = "Not Safe",
...
#we need to create a function to help plotting the percent within target graph
#for cyto and patho. the graph will have the split between IP and AMB
ap_pat_tat_plot <-
...
#This function helps with going through all the facilities at each site for
#each test and spec group to automate the plotting for the efficiency indicator
...
#Here we summarize the data for anatomic pathology in order to calculate the
#defects and defects rate to be used for p-charts
...
#This function creates plots for the p-chart for the anatomic pathology
...
#This function helps with going through all the facilities at each site for
#each test and spec group to automate the plotting for the p-charts.
...
cp_stratified_data_for_plots(tat_data = cp_dashboard_tat_filter,
                             test = "Troponin")
```
...
### Cyto Gyn Specimens
```{r Cyto Gyn Specimens historical data, eval = TRUE, echo = FALSE, warning = FALSE, message = FALSE, fig.height = 5, fig.width = 8, fig.align="center"}
...
cp_stratified_data_for_pcharts(defect_data = cp_dashboard_defect_summary,
                               test = "Troponin")
```
...
### Cyto Gyn Specimens
```{r Cyto Gyn Specimens  pchart, eval = TRUE, echo = FALSE, warning = FALSE, message = FALSE, fig.height = 5, fig.width = 8, fig.align="center", results='asis'}
