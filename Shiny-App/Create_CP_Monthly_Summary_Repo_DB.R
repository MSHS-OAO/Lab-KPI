...
monthly_repo <- readRDS(
  paste0(user_directory,
         "/CP Repositories/MonthlyRepo",
         "/Monthly Repo Dec2020 to Jul2023 as of 08-30-23.RDS")
)

monthly_repo_db <- monthly_repo %>%
  select(-MonthNo, -MonthName, -Year) %>%
  rename(SITE = Site,
         MONTH = MonthRollUp,
         TEST = Test,
         DIVISION = Division,
...
id_primary_keys_1 <- monthly_repo_db %>%
  select(SITE, MONTH, TEST, DETAILED_SETTING, DASHBOARD_PRIORITY) %>%
  distinct()


get_values_monthly_summary <- function(x, table_name){
  
  site <- x[1]
  month <- x[2]
  test <- x[3]
  division <- x[4]
  detailed_setting <- x[5]
  dashboard_setting <- x[6]
  dashboard_priority <- x[7]
  receive_result_target <- x[8]
  collect_result_target <- x[9]
  total_resulted <- x[10]
  receive_time_vol_incl <- x[11]
  collect_time_vol_incl <- x[12]
  total_receive_result_in_target <- x[13]
  total_collect_result_in_target <- x[14]
  total_add_on <- x[15]
  total_missing_collections <- x[16]
  collect_receive_avg <- x[17]
  collect_receive_median <- x[18]
  collect_receive_95 <- x[19]
  receive_result_avg <- x[20]
  receive_result_median <- x[21]
  receive_result_95 <- x[22]
  collect_result_avg <- x[23]
  collect_result_median <- x[24]
  collect_result_95 <- x[25]
...
monthly_summary_temp_table <- "CP_MONTHLY_REPO_TEMP"
monthly_summary_repo_table <- "CP_MONTHLY_REPO"

# test_data <- weekly_repo_db[1:10000, ]
# 
# processed_input_data <- test_data
processed_input_data <- monthly_repo_db
  ...
  inserts <- mclapply(
    mclapply(
      mclapply(split(processed_input_data,
               ...
               as.list),
      as.character),
    FUN = get_values_monthly_summary, monthly_summary_temp_table)
)
...
merge_query <- glue('MERGE INTO "{monthly_summary_repo_table}" RT
                    USING "{monthly_summary_temp_table}" SOURCE_TABLE
                    ON (RT."SITE" = SOURCE_TABLE."SITE" AND
                        RT."MONTH" = SOURCE_TABLE."MONTH" AND
...
truncate_query <- glue('TRUNCATE TABLE "{monthly_summary_temp_table}";')
