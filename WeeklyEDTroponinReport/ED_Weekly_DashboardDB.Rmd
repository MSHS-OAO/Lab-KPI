...
# Data source and change 
target_change <- tibble(
  Date = c("As of 12/31/2024", "As of 12/31/2024", "Starting 01/01/2025", "Starting 01/01/2025"),
  Metric = c("Received to Result", "Collect to Result", "Received to Result", "Collect to Result"),
  Target = c("<= 50 mins", "<= 60 mins", "<= 60 mins", "<= 70 mins")
)
...
previous_year_start <- paste0(previous_year,'-01','-01')
...
cp_hist <- tbl(conn,"LAB_KPI_CP_WEEKLY_VIEW") %>%
  filter(WEEKSTART >= TO_DATE(previous_year_start, 'YYYY-MM-DD')) %>%
  collect()%>%
  rename(Site = SITE,
         WeekStart = WEEKSTART,
         WeekEnd = WEEKEND,
...
# Remove the Receive to Result <= 60   and Collect to Result <=  70 min for the week of 12/29/24-01/04/25
...
MSH and MSQ: Based on daily SCC report. Troponin labs identified by those with Test ID of TROI, HSTRI, TROP, TROPHS. Patient setting based on Clinic Type field and ICU identified using Ward Name field. </br>
...
<h5><i>Target Updates</i></h5>

As of 12/31/2024

- Received to Result: <= 50 mins
- Collect to Result: <= 60 mins

Starting 01/01/2025

- Received to Result: <= 60 mins
- Collect to Result: <= 70 mins
