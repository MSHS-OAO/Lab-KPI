---
  title: "System Lab ED PI Committee - Monthly Dashboard"
...
cp_user_path <- paste0(user_directory, "/CP Repositories/MonthlyRepo/*.*")
  ...
  readRDS(
    file =
      choose.files(default = cp_user_path,
                   caption = "Select Clinical Pathology Monthly Repository"))
...
dashboard_start <- as.Date("1/1/21", format = "%m/%d/%y")
...
repo_median_tat <- cp_hist %>%
  filter(MonthRollUp >= dashboard_start &
...
process_steps_stacked_bar <- function(site, test, setting) {
  ...
  ggplot(process_steps_median_tat,
    ...
    labs(title = paste(site,
                       setting,
                       test,
                       "Median Turnaround Time by Month"),
         x = "Month",
         y = "Time (min)") +
    theme_bw() +
    ...
    geom_text(aes(x = MonthRollUp, y = MedianTAT, label = MedianTAT),
              color = "white",
    ...
    scale_x_date(date_labels = "%m-%y",
                 breaks = sort(unique(process_steps_median_tat$MonthRollUp)),
                 expand = c(0, 1))
}
...
collect_result_stacked_bar <- function(site, test, setting) {
  ...
  ggplot(collect_result_median_tat,
    ...
    scale_y_continuous(expand = c(0, 0, 0.1, 0)) +
    scale_x_date(date_labels = "%m-%y",
                 breaks = sort(unique(collect_result_median_tat$MonthRollUp)),
                 expand = c(0, 1))
}
...
missing_collection_kable <- function(site, test, setting) {
  ...
  missing_collect_summary <- cp_hist %>%
    filter(Site == site &
             Test == test &
             MasterSetting == setting &
             MonthRollUp >= dashboard_start) %>%
    group_by(MonthRollUp) %>%
    ...
    mutate(# Convert week start to text for better table formatting
      MonthRollUp = format(MonthRollUp, "%b-%y"),
    ...
    mutate(Category = ifelse(Metric %in% c("TotalMissingCollections",
           ...
           Metric = ifelse(Metric %in% c("TotalMissingCollections",
                                         "TotalResultedVolume"),
                           "Vol of Labs", "% of Labs")) %>%
    select(Category, Metric, MonthRollUp, value) %>%
    # Pivot wider to get a column for each week
    pivot_wider(names_from = MonthRollUp,
                names_sort = FALSE) %>%
    ungroup()
  ...
  kable(missing_collect_summary,
    ...
    add_header_above(c(" " = 2,
                       "Month" = ncol(missing_collect_summary) - 2),
