...
todays_date <-  as.Date(Sys.Date(), format = "%m/%d/%Y")
...
# tat_targets_ap1 <- data.frame(read_excel(reference_file,
...
all_sites <- c("MSH", "MSQ", "MSB", "MSW", "MSM", "MSSN", "RTC")
hosp_sites <- c("MSH", "MSQ", "MSB", "MSW", "MSM", "MSSN")
...
```{r Custom function for POWERPATH processing}
...
get_max_date_cyto <- glue("SELECT TO_CHAR(MAX(SIGNED_OUT_DATE),'YYYY-MM-DD') AS MAXDATE
...
max_date_cyto <- dbGetQuery(conn,
                           get_max_date_cyto)
max_date_cyto <- max_date_cyto$MAXDATE[[1]]
...
if(as.Date(max_date_cyto)+2 >= todays_date){
  
  file_list_cyto_flag <- FALSE
  
}else{
  cyto_date_range <- seq(from = as.Date(max_date_cyto)+2,
                      to = todays_date,
                      by = "day")


  # Extract SUN pattern and files ----
  pattern_cyto <-  paste0("^(KPI REPORT - RAW DATA V4_V2){1}.*",
                         cyto_date_range,
                         ".xls", collapse = "|")
  file_list_cyto <- list.files(
    path = paste0(user_directory, "/AP & Cytology Signed Cases Reports"),
    pattern = pattern_cyto)

  file_list_cyto_flag <- TRUE
...
```{r Process and Migrate PowerPath Data,warning=FALSE}

if(file_list_cyto_flag){
  for (file in file_list_cyto){
    print(file)
...
if(todays_date-1 > floor_date(todays_date,"month")){
  dates_check <- seq.Date(from = floor_date(todays_date-150,"month"),
                      to = todays_date-1,
                      by = "day")
}
...
scc_dates <- glue("SELECT DISTINCT RESULTDATE
                   FROM LAB_KPI_PREPROCESSED_DAILY
                   WHERE RESULTDATE >=  TRUNC(SYSDATE,'mm') - 151
                   ORDER BY RESULTDATE;")
...
biopsy_dates <- glue("SELECT DISTINCT SIGNED_OUT_DATE
                   FROM LAB_KPI_POWERPATH_CYTO_BIOPSY
                   WHERE SIGNED_OUT_DATE >=  TRUNC(SYSDATE,'mm') - 151
...
cyto_dates_db <- dbGetQuery(conn,
                           biopsy_dates)
cyto_dates_db <- as.Date(cyto_dates_db$SIGNED_OUT_DATE)
...
missing_cyto_dates <- dates_check[!(dates_check %in% cyto_dates_db)]
missing_cyto_dates_file_dates <- missing_cyto_dates+1
...
#       print(paste("Missing File",sun_file))
#     }else{
#       print(sun_file)
#       sun_data <- read_excel(
